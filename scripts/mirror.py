...
webserver.DEFAULT_ICON_TEMPLATE = os.path.join(HTML_DIR, "resources", "templates", "pedal-default.html")
webserver.DEFAULT_SETTINGS_TEMPLATE = os.path.join(HTML_DIR, "resources", "settings.html")
...
async def sync_with_master(app):
    url = f"ws://192.168.88.247/websocket" 
...
if __name__ == "__main__":
    def dummy_cb(*args, **kwargs): pass
    host = SlaveHost(hmi=FakeHMI(init_cb=dummy_cb), prefs=MockPrefs(), msg_callback=dummy_cb)
    ...
    handlers = [
        ...
        (r"/(.*)", tornado.web.StaticFileHandler, {"path": HTML_DIR, "default_filename": "index.html"}),
    ]
